...
scale_fill_manual(values=c("#93C54B", "#4BC5BA", "#7D4BC5", "#C54B56"))
```

### Participants
Histogram with age and gender of participants
```{r}
```
  ...
  DSEML_results %>%
    select(c(valence_E_low_1, valence_D_low_1, valence_M_low_1, valence_H_low_1, valence_E_mid_1, valence_D_mid_1, valence_M_mid_1, valence_H_mid_1, valence_E_high_1, valence_D_high_1, valence_M_high_1, valence_H_high_1)) %>%
      pivot_longer(c("valence_E_low_1", "valence_D_low_1",
        ...
        group_by(Languages) %>% 
          summarise(Values = mean(values_mean))
valence['Valence_degree'] <- c("low", "mid", "high", "low", "mid",
                      "high", "low", "mid", "high", "low", "mid", "high")
valence[1, 1] = "Dutch"
valence[2, 1] = "Dutch"
valence[3, 1] = "Dutch"
valence[4, 1] = "English"
...
### Explanation
Comparing the Spotify valences (high, low, mid) to the perceived valence, etc.
...
scale_fill_manual(values=c("#93C54B", "#121909"))
```
...
scale_fill_manual(values=c("#4BC5BA", "#091917"))
```
...
scale_fill_manual(values=c("#7D4BC5", "#100919"))
```
...
scale_fill_manual(values=c("#C54B56", "#19090B"))
```
...
scale_fill_manual(values=c("#93C54B", "#4BC5BA", "#7D4BC5", "#C54B56", "#19090B"))
```

### chart
```{r}

ggplot(DSEML_results, aes(x=age)) +
geom_histogram(aes(y=..density..), position="identity", alpha=0.5, fill = "red")
...
```{r}
lik_fam <- data.frame(
  familiarity = c(DSEML_results[["familiar_E_low"]], DSEML_results[["familiar_D_mid"]], DSEML_results[["familiar_H_high"]], DSEML_results[["familiar_M_low"]]),
  likeability = c(DSEML_results[["enjoy_E_low"]], DSEML_results[["enjoy_D_mid"]], DSEML_results[["enjoy_H_high"]], DSEML_results[["enjoy_M_low"]]),
  language = c("English", "Dutch", "Hindi", "Mandarin"),
  valence = c(0.206, 0.571, 0.802, 0.206)
  )
ggplot(lik_fam, aes(x=familiarity, y=likeability, size = valence, color = language)) +
    geom_point(alpha=0.7) +
    scale_size(range = c(0, 1), name="Population (M)")
```
...
Used_songs <- data.frame (
  Title = c("Mojito", "Leef nu het kan", "Walk of life", "Mummy Nu Pasand (From 'Jai Mummy Di')", "寬恕", "Respect", "Don't look back in anger", "Jagga Jiteya", "愛的就是你", "Treur niet (Ode aan het leven)", "Maybe tomorrow", "Shubhaarambh"),
